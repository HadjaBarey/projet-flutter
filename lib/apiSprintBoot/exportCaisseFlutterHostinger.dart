...
Future<List<JournalCaisseModel>> getFilteredOperationsFromHive(String dateControle) async {
  final box = await openBoxSafe<JournalCaisseModel>('todobos7');
  final allJournalCaisse = box.values.toList();
  return allJournalCaisse.where((op) => op.dateJournal == dateControle).toList();
}
...
Future<bool> deleteDataBeforeImport({
  ...
  final deleteUrl = 'https://kadoussconnect.com/transfertflutter/backend/deleteCaisse.php';
...
Future<bool> saveDataToPhp({
  required BuildContext context,
  required List<JournalCaisseModel> journal,
  required EntrepriseModel entreprise,
}) async {
  final postUrl = 'https://kadoussconnect.com/transfertflutter/backend/saveCaisse.php';

  final userKey = await getUserKeyFromHiveHos();
  final userKeyValue = userKey?.numeroaleatoire ?? '';

  final jsonToSend = journal.map((op) {
    final map = op.toJson();
...
Future<void> exportCaisse(BuildContext context) async {
  ...
  final journalsFiltrees = await getFilteredOperationsFromHive(entreprise.DateControle);
  if (journalsFiltrees.isEmpty) {
